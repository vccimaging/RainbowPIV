function [mang, stdang, mepe]=flowAngErr(tu, tv, tw, u, v, w, bord, varargin)
...
stu=tu(bord+1:end-bord,bord+1:end-bord,bord+1:end-bord);
stv=tv(bord+1:end-bord,bord+1:end-bord,bord+1:end-bord);
stw=tw(bord+1:end-bord,bord+1:end-bord,bord+1:end-bord);
...
ind2=find( abs(stu(:))>smallflow | abs(stv(:)>smallflow) | abs(stw(:))>smallflow ); 
%length(ind2)
n=1.0./sqrt(su(ind2).^2+sv(ind2).^2+sw(ind2).^2+1);
un=su(ind2).*n;
vn=sv(ind2).*n;
wn=sw(ind2).*n;

tn=1./sqrt(stu(ind2).^2+stv(ind2).^2+stw(ind2).^2+1);
tun=stu(ind2).*tn;
tvn=stv(ind2).*tn;
twn=stw(ind2).*tn;
ang=acos(un.*tun+vn.*tvn+wn.*twn+(n.*tn));
mang=mean(ang);
mang=mang*180/pi;
...
if nargout == 3
    epe = sqrt((stu-su).^2 + (stv-sv).^2 + (stw-sw).^2);
    epe = epe(ind2);
    mepe = mean(epe(:));
end;
